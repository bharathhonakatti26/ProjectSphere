...
const TeacherDiscovery = () => {
  ...
  const [mentorOpened, { open: openMentor, close: closeMentor }] = useDisclosure(false);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Show all public projects except those created by the current user
  ...
  const handleOfferMentorship = async () => {
    if (!selectedProject) return;
    setSubmitting(true);
    try {
      // Teachers can request to join as mentors
      await projectAPI.requestToJoin(selectedProject._id, mentorMessage);
      showSuccess('Mentorship offer sent!', 'The project team will review your offer');
      closeMentor();
      setMentorMessage('');
          ...
          <Stack gap="md" h="100%">
            ...
            <Group justify="space-between" align="flex-start">
              ...
              <Group gap="xs">
                <Badge 
                  variant="light" 
                  color={project.type === 'STUDENT_INITIATED' ? 'cyan' : 'violet'}
                  leftSection={project.type === 'STUDENT_INITIATED' ? <IconRocket size={12} /> : <IconSchool size={12} />}
                >
                  {project.type === 'STUDENT_INITIATED' ? 'Student' : 'Classroom'}
                </Badge>
                <Badge 
                  variant="light" 
                  color="green"
            ...
            <Group gap="xs">
              ...
              <Badge size="xs" variant="dot" color="cyan">Student</Badge>
            </Group>
            ...
            <Group gap="xs">
              ...
              <Button
                ...
                onClick={() => openMentorModal(project)}
                style={{ flex: 1 }}
              >
                Offer Mentorship
              </Button>
  ...
  return (
    <Stack gap="xl">
      ...
      >
        <Stack gap="xs">
          <Title order={2}>Discover Student Projects</Title>
          <Text c="dimmed">Browse public projects from students and offer mentorship</Text>
        </Stack>
      </motion.div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="No public student projects available right now. Check back later!"
        />
      )}
      ...
      <Modal
        opened={mentorOpened}
        onClose={closeMentor}
        title={<Text size="lg" fw={600}>Offer Mentorship</Text>}
        centered
      >
        <Stack gap="md">
          <Text size="sm" c="dimmed">
            Send a mentorship offer to <strong>{selectedProject?.title}</strong>. 
            The project team will review your offer.
          </Text>
          ...
          <Group justify="flex-end">
            <Button variant="subtle" onClick={closeMentor}>
              Cancel
            </Button>
            <Button
              onClick={handleOfferMentorship}
              loading={submitting}
          ...
          <Stack gap="md">
            ...
            <Group gap="md">
              ...
              <Box>
                ...
                <Text size="sm" c="dimmed">Project Creator (Student)</Text>
              </Box>
              <Button
                variant="light"
                size="xs"
            ...
            <Group>
              <Badge 
                variant="light" 
                color={selectedProject.type === 'STUDENT_INITIATED' ? 'cyan' : 'violet'}
                size="lg"
              >
                {selectedProject.type === 'STUDENT_INITIATED' ? 'Student Initiated Project' : 'Classroom Based Project'}
              </Badge>
            </Group>
            ...
            <Box>
              <Text size="sm" fw={600} mb="xs">
                Team Members ({selectedProject.members?.filter(m => m.status === 'ACTIVE')?.length || 0})
              </Text>
              <Group gap="xs">
            ...
            <Group justify="flex-end" mt="md">
              <Button
                ...
                onClick={() => {
                  closeView();
                  openMentorModal(selectedProject);
                }}
              >
                Offer Mentorship
              </Button>
